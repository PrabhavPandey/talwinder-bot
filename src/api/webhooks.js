...
async function processIncomingMessage(phoneNumber, text, images, userName, messageId) {
  try {
    ...
    const systemPrompt = `YOU ARE GROUNDED IN THE FOLLOWING PRODUCT DOCUMENTS. USE THIS CONTEXT TO INFORM YOUR RESPONSES ABOUT TAL, ROUND1, AND GRAPEVINE:\n\n${productContext}\n\n${prompts.systemPrompt}\n\nUSER NAME: ${displayName}\nUSER MEMORY:\n${memoryContext}\n\nCURRENT TIME: ${new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })}`;
...
function stripEmojis(text) {
  // 1. Remove emojis
  let result = text.replace(/[\u{1F600}-\u{1F64F}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{1F1E0}-\u{1F1FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{FE00}-\u{FE0F}\u{1F900}-\u{1F9FF}\u{1FA00}-\u{1FA6F}\u{1FA70}-\u{1FAFF}\u{200D}\u{20E3}\u{FE0F}]/gu, '');

  // 2. Collapse horizontal whitespace but PRESERVE newlines (\n)
